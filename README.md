...
### Импорт готовых договоров

В разделе «Документы → Загрузить документ» договоры принимаются в PDF, DOCX и DOC, счета — в PDF (до 20 МБ, PDF-копия до 20 страниц). Для Word серверу нужен LibreOffice: команда `soffice` в PATH либо абсолютный путь в `CRM_SOFFICE_PATH`. Конвертация выполняется с отдельным временным профилем и ограничением 60 секунд. Оригинал Word сохраняется отдельно; просмотр и подписание используют созданную PDF-копию. Перед подписью её оформление нужно проверить.

Образ `apps/api/Dockerfile` устанавливает LibreOffice Writer и шрифты, собирает кабинет и проверяет преобразование DOCX/DOC в PDF внутри образа. `render.yaml` использует Docker. Для существующего сервиса нужно применить смену среды в Render (см. `docs/deployment.md`); обычная публикация кода в старой среде Node не устанавливает LibreOffice. При запуске вне Docker LibreOffice нужно установить отдельно. Изменение локального кода не обновляет работающий сайт.

После проверки распознанных полей и сохранения договора пустые реквизиты исполнителя переносятся в профиль организации, если БИН не конфликтует с существующим. Заполненные поля не заменяются. Для ранее импортированных договоров кнопка «Заполнить реквизиты из договора» использует сохранённые при импорте данные; файл повторно загружать не нужно. Действие доступно с теми же правами, что редактирование реквизитов, и фиксируется в журнале.
