...
Образ `apps/api/Dockerfile` устанавливает LibreOffice Writer и шрифты для конвертации. При запуске вне Docker LibreOffice нужно установить отдельно. Текущий `render.yaml` использует среду Node: сам по себе этот Blueprint не устанавливает LibreOffice. Перед публикацией Word-импорта убедитесь, что `soffice --version` работает именно в среде API, и проверьте загрузку DOCX и DOC. Изменение локального кода не обновляет работающий сайт.
