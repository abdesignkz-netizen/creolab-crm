...
export function AvrEditorPage(){
  ...
  async function loadDeal(dealId:string,existing?:any){
    ...
    try{
      ...
      setContext(record?.externalSystem === "BASQAR" ? {...ctx, organization:{...record.source.seller}, company:{...record.source.buyer}} : ctx);setDoc(record||null);setForm(record?{documentDate:record.documentDate.slice(0,10),items:toEditorItems(record.source?.items||[])}:{...empty,items:toEditorItems(ctx.items)});setDirty(false);setIssues({});setEditingBuyer(false);setPhase("");
  ...
  async function signLocally(){
    ...
    try {
      ...
      await reloadLocalSigning(record.id);
      notifySaved("АВР подписан исполнителем. Теперь можно создать ссылку заказчику.");
  ...
  async function sendLocalLink(){
    ...
    notifySaved("Ссылка готова. Скопируйте и передайте её заказчику.");
  }
  async function cancelLocalSigning(){
    if(!doc)return;
    if(!window.confirm("Отменить подпись АВР и вернуть документ в исправление? Подпись исполнителя и ссылка заказчику будут аннулированы."))return;
    await api.cancelAvrSigning(doc.id);
    await reloadLocalSigning(doc.id);
    setLocalSigning(null);setBuyerLink("");setDirty(false);
    notifySaved("Подписание отменено. АВР снова можно исправлять.");
  }
  async function deleteCurrentAvr(){
    if(!doc)return;
    if(!window.confirm("Удалить этот АВР? Действие нельзя отменить."))return;
    await api.deleteAvr(doc.id);
    notifySaved("АВР удалён");
  ...
  async function previewAvrPdf(){
    ...
    if (!/^application\/pdf(?:$|;)/i.test(file.blob.type)) throw new Error("Сервер вернул документ не в формате PDF");
