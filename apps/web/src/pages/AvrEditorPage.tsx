...
import { avrEditorAmounts, avrEditorSchema, type AvrEditorInput } from "@creolab/contracts";
...
export function AvrEditorPage(){
  ...
  async function loadDeal(dealId:string,existing?:any){
    ...
    try{
      ...
      setContext(ctx);setDoc(record||null);setForm(record?{documentDate:record.documentDate.slice(0,10),items:record.source?.items||[]}:{...empty,items:ctx.items});setDirty(false);setIssues({});setEditingBuyer(false);setPhase("");
  ...
  async function action(fn:()=>Promise<void>){if(flight.current)return;flight.current=true;setBusy(true);setError("");try{await fn();}catch(e:any){setError(e.message||"Не удалось выполнить действие");setPhase(e.code==="USER_CANCELLED"?"":"ERROR");const detail=e.body?.details;if(e.body?.field_errors)setIssues(Object.fromEntries(Object.entries(e.body.field_errors).map(([k,v])=>[editingBuyer?`customer.${k}`:k,String(v)])));if(detail?.missingFields)setIssues(Object.fromEntries(detail.missingFields.map((f:string)=>[f,detail.missingFieldLabels?.[f]||f])));}finally{flight.current=false;setBusy(false);}}
  async function connect(){
    ...
    if(current.system.provider==="live")await connectEsfAuthTicket(iin);
    ...
    const updated:any=await api.esfConnection();if(!updated.connection.sessionActive)throw new Error("Авторизация ИС ЭСФ не завершена");setConnection(updated);setConnected(true);setIssues({});
  }
  async function check(record:any){
    const validated:any=await api.validateElectronicDocument(record.id);setDoc(validated.document);setWarnings(validated.warnings||warnings);setIssues({});setPhase("VALIDATED");
  }
  ...
  const fieldError=(key:string)=><span className="error" role="alert">{issues[key]}</span>;
  return <section className="avr-editor"><div className="row"><div><Link to="/documents?kind=AVR">АВР</Link><h2>{doc?`АВР ${doc.number}`:"Создание АВР"}</h2></div><span className={`document-status status-${phase||(doc?.errorCode?"ERROR":doc?.status)||"DRAFT"}`}>{labels[phase||(doc?.errorCode?"ERROR":doc?.status)||"DRAFT"]||doc?.status}</span></div>
    ...
    {!context?<div className="panel"><h3>Сделка</h3><div className="actions"><button className={filter==="ready"?"btn":"btn secondary"} onClick={()=>setFilter("ready")}>Готовы к закрытию</button><button className={filter==="all"?"btn":"btn secondary"} onClick={()=>setFilter("all")}>Все незакрытые</button></div><label>Найти сделку<input value={q} onChange={e=>setQ(e.target.value)} placeholder="Название или компания"/></label>{!deals.length?<p>Подходящих сделок нет. Попробуйте фильтр «Все незакрытые».</p>:null}{deals.map(d=><div className="card" key={d.id}><b>{d.title} — {d.companyName||d.contactName}</b><p>Сделка #{d.number} · {money(d.amount)} ₸ · {d.stage} · {d.responsible||"Ответственный не назначен"}</p><p>{d.paymentStatus==="PAID"?"Оплачено":"Оплата не подтверждена"}{d.paidAt?` · ${new Date(d.paidAt).toLocaleDateString("ru-RU")}`:""}</p><p className={d.ready?"ok":"pdf-import-warnings"}>{d.ready?"Готова к закрытию":d.reasons.join("; ")}</p><p>{d.documentState.label}</p><button className="btn secondary" disabled={busy} onClick={()=>void loadDeal(d.id)}>{d.documentId?"Открыть АВР":"Выбрать"}</button></div>)}</div>:<>
      ...
      <div className="panel"><h3>Позиции АВР</h3>{fieldError("deal.items")}{form.items.map((r,i)=><fieldset disabled={busy||immutable} className="avr-line" key={i}><label>Работа / услуга<input value={r.name} onChange={e=>item(i,{name:e.target.value})}/>{fieldError(`items.${i}.name`)}</label><label>Количество<input type="number" min="0.001" step="0.001" value={r.quantity} onChange={e=>item(i,{quantity:Number(e.target.value)})}/>{fieldError(`items.${i}.quantity`)}</label><label>Ед.<input value={r.unit} onChange={e=>item(i,{unit:e.target.value})}/>{fieldError(`items.${i}.unit`)}</label><label>Цена без НДС<input type="number" min="0" step="0.01" value={r.unitPrice} onChange={e=>item(i,{unitPrice:Number(e.target.value)})}/>{fieldError(`items.${i}.unitPrice`)}</label><label>НДС, %<input type="number" min="0" max="100" step="0.01" value={r.vatRate} onChange={e=>item(i,{vatRate:Number(e.target.value)})}/>{fieldError(`items.${i}.vatRate`)}</label><p>{money(totals?.rows[i].totalAmount)} ₸</p><button className="btn secondary" onClick={()=>edit({items:form.items.filter((_,n)=>n!==i)})}>Удалить</button></fieldset>)}<button className="btn secondary" disabled={busy||immutable} onClick={()=>edit({items:[...form.items,{name:"",quantity:1,unit:"услуга",unitPrice:0,vatRate:Number(context.organization?.defaultVatRate||0)}]})}>+ Добавить позицию</button><p>Без НДС: {money(totals?.totals.amountWithoutVat)} ₸ · НДС: {money(totals?.totals.vatAmount)} ₸ · <b>Итого: {money(totals?.totals.totalAmount)} ₸</b></p>{fieldError("")}{fieldError("deal.amount")}<button className="btn" disabled={busy||immutable} onClick={()=>void action(async()=>{await save();notifySaved("Черновик АВР сохранён");})}>Сохранить черновик</button></div>
      <div className="panel"><h3>Подписание и отправка</h3><p className={connected?"ok":"muted"}>{connected?"NCALayer подключён":"NCALayer подключится при нажатии «Подписать и отправить АВР»"}</p>{connection?<p>{connection.organization?.legalName} · БИН {connection.connection?.organizationBin} · Сертификат {connection.connection?.certificateSerial}</p>:null}<label>ИИН пользователя для авторизации ЭСФ<input value={iin} maxLength={12} onChange={e=>setIin(e.target.value.replace(/\D/g,""))}/></label>{fieldError("ncalayer")}<div className="actions"><button className="btn secondary" disabled={busy||immutable} onClick={()=>void action(async()=>{const record=dirty||!doc?await save():doc;await check(record);})}>Проверить документ</button><button className="btn" disabled={busy||immutable||editingBuyer} onClick={()=>void action(async()=>{const record=dirty||!doc?await save():doc;await check(record);await connect();const result:any=await signAndSendEsfDocument(record.id,setPhase);setDoc(result.sent.document);setPhase("");notifySaved("АВР отправлен");})}>Подписать и отправить АВР</button>{doc?.externalId?<button className="btn secondary" disabled={busy} onClick={()=>void action(async()=>{await api.refreshElectronicDocumentEsf(doc.id);const r:any=await api.request(`/api/v1/electronic-documents/${doc.id}`);setDoc(r.document);})}>Обновить статус</button>:null}</div>{doc?.errorMessage?<p className="error">{doc.errorMessage}</p>:null}{doc?.externalId?<p>Номер регистрации: {doc.externalNumber||doc.externalId}</p>:null}</div>
    </>}
  </section>;
}
