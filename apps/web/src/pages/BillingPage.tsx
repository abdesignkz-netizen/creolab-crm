...
export function BillingPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice(free ? "BasQar Free подключён без оплаты." : enterprise ? "Запрос на индивидуальные условия отправлен." : "Запрос на подключение отправлен. Доступ откроется после подтверждения оплаты.");
      await loadBilling();
  ...
  return <section className="billing-page stack">
    ...
    <section className="panel stack"><h3>Дополнительные ресурсы</h3>
      <p className="muted">Для подходящего платного тарифа. Дополнительные ресурсы не открывают функции другого тарифа.</p>
      <div className="billing-addons">{extras.map(item => <div className="billing-addon" key={item.code}><div><b>{item.name}</b><p>{item.description}</p><p>{item.code === "ADDON_INTEGRATION" ? "от " : ""}{formatKzt(catalogPrice(item, period))} / {item.chargeType === "ONE_TIME" ? "разово" : periodLabel}</p></div></div>)}</div>
      {(data?.addOns || []).length ? <p>Купленные ресурсы: {(data.addOns as Array<{code:string;qty:number}>).map(row => `${items.find(item=>item.code===row.code)?.name || CATALOG_BY_CODE[row.code]?.name || "Прежний модуль"} × ${row.qty}`).join(", ")}</p> : null}
      {data?.accessBreakdown?.baseLimits ? <div className="billing-table-scroll"><table><thead><tr><th>Ресурс</th><th>В тарифе</th><th>Итоговый лимит</th></tr></thead><tbody>{(data.usage || []).map((row: {key:string;label:string;cap:number}) => <tr key={row.key}><th>{row.label}</th><td>{data.accessBreakdown.baseLimits[row.key] ?? "—"}</td><td>{row.cap < 0 ? "Без квоты" : row.cap}</td></tr>)}</tbody></table></div> : null}
    </section>
    <div ref={selection} className="billing-selection" tabIndex={-1}>
      {planCode ? <div className="panel stack">
        ...
        {constructorOpen && !enterprise && !free ? <div className="stack">
          <h4>Дополнительные ресурсы</h4><p className="muted">Ресурсы увеличивают лимиты тарифа и не открывают функции другого тарифа.</p>
