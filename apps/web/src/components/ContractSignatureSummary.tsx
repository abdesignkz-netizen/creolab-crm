...
export function ContractSignatureSummary({ signed, signers, verificationUrl, download, sellerName, buyerName }: {
  ...
  async function save(format: "pdf" | "zip") {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : "Не удалось скачать договор"); }
    finally { setBusy(false); }
  }
  if (!signers.length && !signed) return null;
  return <section className="panel" aria-label="Подписи договора">
    <h3>{signed ? "Подписан обеими сторонами" : "Подписи договора"}</h3>
    ...
    {signed ? <>
      ...
      <p className="muted">PDF содержит договор и лист подписания с QR-кодом. В архиве — неизменённый оригинал, обе ЭЦП и сведения о подписании.</p>
    </> : null}
