...
export function resolveEsfBaseUrl(input?: {
  ...
  // TEST is pinned to the official origin and path, including when overrides contain credentials.
  if (esfEnv === "test") return ESF_OFFICIAL.testBaseUrl;
  return baseUrl.replace(/\/$/, "");
}

export function assertTestEndpointNotProduction(config: { esfEnv: EsfEnv; baseUrl: string }) {
  if (config.esfEnv !== "test") return;
  const official = new URL(ESF_OFFICIAL.testBaseUrl);
  let url: URL;
  try { url = new URL(config.baseUrl); } catch { throw new Error("ESF_ENV=test: некорректный endpoint"); }
  if (url.origin !== official.origin || url.username || url.password || url.search || url.hash ||
      !(url.pathname === official.pathname || url.pathname.startsWith(`${official.pathname}/`))) {
    throw new Error("ESF_ENV=test запрещает production и любой endpoint вне официального TEST");
  }
}
