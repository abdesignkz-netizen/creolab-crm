...
describe('Free tenant quotas and preserved pricing', () => {
  ...
  it('automatically activates only new self registrations with exact limits and no paid features', async () => {
    ...
    for (const [key,value] of Object.entries({ USERS:1, CLIENTS:15, ACTIVE_DEALS:10, MONTHLY_LEADS:20, PIPELINES:1, DATABASE_MB:50, FILE_STORAGE_MB:0 })) assert.equal(access.limits[key],value);
    for (const feature of ['TASKS','CLIENTS','COMPANIES','LEADS','DEALS'] as const) assert.equal(access.entitlements[feature],true);
    for (const feature of ['AI_MANAGER','AI_CONTROL','WHATSAPP','IMPORT','EXPORT','FILE_STORAGE','WORKFLOWS','DOCUMENTS','API_ACCESS'] as const) assert.equal(access.entitlements[feature],false);
  ...
  it('serializes concurrent creates: 15 clients, second user rejected, existing data readable', async () => {
    const tenant = await freeTenant();
    const results = await Promise.allSettled(Array.from({length:16}, (_,i) => db.contact.create({data:{tenantId:tenant.id,name:`Client ${i}`}})));
    assert.equal(results.filter(row=>row.status==='fulfilled').length,15);
    assert.equal(await db.contact.count({where:{tenantId:tenant.id}}),15);
    assert.equal(await getUsage(db,tenant.id,'CLIENTS'),15);
  ...
  it('counts active deals, keeps closed history, prevents reopening beyond quota', async () => {
    ...
    const deals = [];
    for(let i=0;i<10;i++) deals.push(await db.deal.create({data:{tenantId:tenant.id,contactId:contact.id,stageId:stage.id,title:`Order ${i}`}}));
    await assert.rejects(db.deal.create({data:{tenantId:tenant.id,contactId:contact.id,stageId:stage.id,title:'11'}}), /BASQAR_LIMIT:ACTIVE_DEALS/);
    ...
    assert.equal(await db.deal.count({where:{tenantId:tenant.id}}),11);
  });
  ...
  it('manual tasks work without AI and DB/files are independent with no data deletion', async () => {
    ...
    await assert.rejects(db.attachment.create({data:{tenantId:tenant.id,parentType:'task',parentId:task.id,storageKey:'test',fileName:'x',mimeType:'text/plain',sizeBytes:1}}),/BASQAR_LIMIT:FILE_STORAGE_MB/);
    await db.tenantUsage.update({where:{tenantId:tenant.id},data:{databaseBytes:BigInt(50*1048576)}});
  ...
  it('keeps calendar-month renewals and unlimited addon limits stable', async () => {
    ...
    const quote = await quoteSubscription(db,{planCode:'CRM_BUSINESS',addOns:[{code:'ADDON_INTEGRATION'}]});
  ...
  it('preserves legacy and uses backend price, compatible recommendations, immutable snapshots', async () => {
    ...
    const quote = await quoteSubscription(db,{planCode:'BUNDLE_CRM_AI',addOns:[{code:'ADDON_USER',qty:2},{code:'ADDON_WHATSAPP',qty:1}]});
    assert.equal(quote.baseAmountMinor,49900); assert.equal(quote.finalAmountMinor,67600); assert.equal(quote.limits.USERS,12);
    assert.equal(quote.recommendation,null);
    const expensive = await quoteSubscription(db,{planCode:'CRM_BUSINESS',addOns:[{code:'ADDON_AI_BUSINESS'},{code:'ADDON_CONTROL'},{code:'ADDON_WHATSAPP'}]});
    assert.equal(expensive.recommendation?.code,'BUNDLE_FULL');
    ...
    const tenant = await freeTenant();
    await activateSubscription(db,{tenantId:tenant.id,planCode:'BUNDLE_CRM_AI',approvedSnapshot:quote.snapshot,amountMinor:quote.finalAmountMinor});
    await db.plan.update({where:{code:'BUNDLE_CRM_AI'},data:{monthlyPriceMinor:999999,limitsJson:{USERS:1}}});
