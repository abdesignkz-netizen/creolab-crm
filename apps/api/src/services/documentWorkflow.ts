...
export function documentWorkflowState(docs: Array<{type:string;status:string;externalStatus?:string|null;externalSystem?:string|null;externalId?:string|null;errorCode?:string|null}>) {
  ...
  const code=error?"ERROR":closed?"CLOSED":sent(esf)?"ESF_SENT":accepted(avr)?"AVR_ACCEPTED":sent(avr)?"AVR_SENT":avr?.status==="SIGNED"&&avr.externalSystem==="BASQAR"?"AVR_SIGNED":avr?.status==="PARTIALLY_SIGNED"?"AVR_PARTIALLY_SIGNED":avr?.status==="PENDING_SIGNATURE"?"AVR_PENDING_SIGNATURE":avr?.status==="SENDING"?"AVR_SENDING":avr?.status==="VALIDATED"?"AVR_READY":avr?"AVR_DRAFT":"AVR_MISSING";
  const labels:Record<string,string>={AVR_SIGNED:"АВР подписан обеими сторонами",AVR_PARTIALLY_SIGNED:"АВР ожидает подписи заказчика",AVR_PENDING_SIGNATURE:"АВР на подписи в BasQar",ERROR:"Ошибка документов",CLOSED:"Документы закрыты",ESF_SENT:"ЭСФ отправлен",AVR_ACCEPTED:"АВР принят · требуется ЭСФ",AVR_SENT:"АВР отправлен · требуется ЭСФ",AVR_SENDING:"АВР отправляется",AVR_READY:"АВР готов к подписи",AVR_DRAFT:"АВР черновик",AVR_MISSING:"АВР не создан"};
  return {code,label:labels[code],closed,finalAvr:sent(avr)||(avr?.status==="SIGNED"&&avr.externalSystem==="BASQAR"),needsEsf:sent(avr)&&!sent(esf),error};
}
...
export async function countDocumentClosing(prisma:PrismaClient,tenantId:string,scope:Prisma.DealWhereInput={}) {
  ...
  const deals=await prisma.deal.findMany({where:{...scope,tenantId,outcome:{not:"lost"}},select:{paymentStatus:true,electronicDocuments:{orderBy:{createdAt:"desc"},select:{type:true,status:true,externalId:true,externalSystem:true,externalStatus:true,errorCode:true}}}});
