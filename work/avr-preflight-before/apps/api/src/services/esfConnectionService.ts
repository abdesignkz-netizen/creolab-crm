...
export function describeAvrPocReadiness(
  ...
  if (row?.environment && row.environment !== "test") reasons.push("Сессия создана не в TEST");
...
export async function getEsfConnection(prisma: PrismaClient, auth: AuthContext) {
  ...
  return {
    ...
    wsseRequired: config.provider !== "mock",
    ncalayer: {
...
export async function connectEsf(
  ...
  try {
    certificate = resolveAuthCertificate({
      ...
      authCertificatePem: raw.authCertificatePem ? String(raw.authCertificatePem) : undefined,
    });
  } catch (error) {
        ...
        data: {
          ...
          lastErrorMessage:
            session.code === "esf_wsse_required"
              ? "ИС ЭСФ требует пароль кабинета. Это не PIN ЭЦП."
              : session.message,
        },
      });
      return {
