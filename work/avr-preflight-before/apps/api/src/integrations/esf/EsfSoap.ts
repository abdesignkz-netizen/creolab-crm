...
import { ESF_OFFICIAL } from "./EsfConfig.ts";
...
export function isWsseCredentialFault(input: { faultstring?: string; description?: string; body?: string; status?: number }) {
  const text = `${input.faultstring || ""} ${input.description || ""} ${input.body || ""}`.toLowerCase();
...
export async function postSoap(url: string, xml: string, options?: { timeoutMs?: number }) {
