...
Object.assign(process.env, {
  ...
  ALLOWED_ORIGINS: "http://127.0.0.1:4297", CRM_INLINE_AUTOMATION: "0",
});
...
const server = app.listen(4298, "127.0.0.1");
const { createServer } = await import("vite");
const web = await createServer({ root: path.resolve("apps/web"), configFile: path.resolve("apps/web/vite.config.ts"), server: {
  host: "127.0.0.1", port: 4297, strictPort: true, proxy: { "/api": "http://127.0.0.1:4298", "/public": "http://127.0.0.1:4298", "/health": "http://127.0.0.1:4298" },
} });
await web.listen();
console.log("ISOLATED_UI_READY http://127.0.0.1:4297");
async function stop() { await web.close(); await new Promise(resolve => server.close(resolve)); await prisma.$disconnect(); rmSync(scratch, { recursive: true, force: true }); process.exit(0); }
